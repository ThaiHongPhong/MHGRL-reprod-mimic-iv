...
def get_top_k_results(tmp, train_tmp, test_y, train_y, k):
    ...
    if k > len(train_tmp):
        raise ValueError(f'k={k} exceeds the {len(train_tmp)} training records')

    distances = pairwise_distances(tmp, train_tmp)
    # Stable sorting makes nearest-neighbour tie handling reproducible.
    top_train_idx = np.argsort(distances, axis=1, kind='stable')[:, :k]

    predict_labels = []
    for train_idx in top_train_idx:
        candidates = [train_y[t_idx] for t_idx in train_idx]
        counts = collections.Counter(candidates)
        best_count = max(counts.values())
        tied_labels = {label for label, count in counts.items() if count == best_count}
        # If the vote is tied, prefer the class of the closest neighbour.
        p_label = next(label for label in candidates if label in tied_labels)
