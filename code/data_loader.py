...
def load_rel(rel_dir, pmi_threshold=1.0):
    rel_dir = Path(rel_dir)
    # read the relations from the file
    code_dtypes = {'head ent': str, 'tail ent': str}
    diag_proce_df = pd.read_csv(
        rel_dir / 'diag_proce_rel.csv', sep='\t', dtype=code_dtypes)
    diag_proce_df = diag_proce_df[diag_proce_df['pmi'] > pmi_threshold]
    diag_pres_df = pd.read_csv(
        rel_dir / 'diag_pres_rel.csv', sep='\t', dtype=code_dtypes)
    diag_pres_df = diag_pres_df[diag_pres_df['pmi'] > pmi_threshold]
    proce_pres_df = pd.read_csv(
        rel_dir / 'proce_pres_rel.csv', sep='\t', dtype=code_dtypes)
    proce_pres_df = proce_pres_df[proce_pres_df['pmi'] > pmi_threshold]

    local_mapping = rel_dir / 'ndc2rxnorm_mapping.txt'
    ndc2rxnorm_file = (
        local_mapping if local_mapping.exists()
        else REPO_ROOT / 'data' / 'ndc_atc' / 'ndc2rxnorm_mapping.txt'
    )
    with ndc2rxnorm_file.open('r', encoding='utf-8') as f:
        ndc2rxnorm = ast.literal_eval(f.read())
...
def construct_EHR_pairs_dataloader(processed_file, tokenizer, ehr_file, label_file,
                                   rel_infos, shuffle=True, batch_size=1,
...
def construct_dataloder(tokenizer, processed_file, ehr_file, rel_infos,
                        batch_size=1, disease_prediction=False):
    """
...
def load_ehr_infos(ehr_file, tokenizer, rel_infos, disease_prediction=False):
    # Initialize dictionary to store EHR information
    ehr_infos = {}
    code_dtypes = {
        'ATC': str, 'ICD9_DIAG': str, 'ICD9_PROCE': str, 'disease': str
    }
    ehr_df = pd.read_csv(ehr_file, dtype=code_dtypes)
    ...
    for _, row in tqdm(ehr_df.iterrows()):
        ...
        hadm_id = row['HADM_ID']
        diag_codes = sorted(set(row['ICD9_DIAG'].split(',')))
        proce_codes = sorted(set(row['ICD9_PROCE'].split(',')))
        atc_codes = sorted(set(row['ATC'].split(',')))
...
def load_ehr_pairs(label_file):
    ...
    for row in label_df.itertuples(index=False, name=None):
...
def get_cohorts(test_ehr_file, valid_ehr_file, train_ehr_file):
    train_df, test_df, valid_df = (
        pd.read_csv(train_ehr_file, dtype={'disease': str}),
        pd.read_csv(test_ehr_file, dtype={'disease': str}),
        pd.read_csv(valid_ehr_file, dtype={'disease': str}),
    )
    # Use one deterministic mapping shared by all splits.  The original code
    # built this list only from test, which fails when a cohort is absent there.
    diseases = sorted(set(pd.concat([
        train_df['disease'], valid_df['disease'], test_df['disease']
    ]).values))
...
def generate_cohort_data(ehr_file):
    ...
    admission_df = pd.read_csv(ehr_file, dtype={'disease': str})
...
def rxnorm_to_atc_mapping(ndc2atc_file_path):
    rxnorm2atc = pd.read_csv(ndc2atc_file_path)
...
def load_dataset(config, train_processed_file, test_processed_file, valid_cluster_processed_file,
    ...
    base_dir = Path(config.data_dir).resolve() if config.data_dir else REPO_ROOT / 'data' / dataset
    vocab_file = base_dir / 'vocab.pkl'

    required_artifacts = [
        'vocab.pkl', 'train_admissions.csv', 'valid_admissions.csv',
        'test_admissions.csv', 'train_label.csv', 'diag_proce_rel.csv',
        'diag_pres_rel.csv', 'proce_pres_rel.csv'
    ]
    missing = [name for name in required_artifacts if not (base_dir / name).exists()]
    if missing:
        raise FileNotFoundError(
            f"Missing MHGRL artifacts in {base_dir}: {', '.join(missing)}. "
            "Run data/preprocess_mimic4.py first for MIMIC-IV."
        )

    rel_infos = load_rel(base_dir, pmi_threshold=config.pmi_threshold)
    ...
    diag_embeddings, proce_embeddings, atc_embeddings = load_text_embeddings(
        str(REPO_ROOT / 'data' / 'chatgpt_desc') + os.sep)
    ...
    train_ehr_file, train_label_file = (
        base_dir / 'train_admissions.csv', base_dir / 'train_label.csv')
    valid_ehr_file, valid_label_file = (
        base_dir / 'valid_admissions.csv', base_dir / 'valid_label.csv')
    ...
    test_dataloader, _ = construct_dataloder(tokenizer, test_processed_file, test_ehr_file, rel_infos, batch_size=batch_size,
                                             disease_prediction=False)
    valid_cluster_dataloader, _ = construct_dataloder(tokenizer, valid_cluster_processed_file, valid_ehr_file, rel_infos, batch_size=batch_size,
                                                      disease_prediction=False)

    valid_knn_dataloader, _ = construct_dataloder(tokenizer, valid_knn_processed_file, valid_ehr_file, rel_infos, batch_size=batch_size,
                                                  disease_prediction=True)
    test_knn_dataloader, _ = construct_dataloder(tokenizer, test_knn_processed_file, test_ehr_file, rel_infos, batch_size=batch_size,
                                                 disease_prediction=True)
    train_input_dataloader, _ = construct_dataloder(tokenizer, train_input_file, train_ehr_file, rel_infos, batch_size=batch_size,
                                                    disease_prediction=True)
